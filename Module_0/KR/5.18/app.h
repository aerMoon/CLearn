...
/* dirdcl: разбор собственно объ€вител€ */
void dlrdcl(void)
{
    int type;
    if (tokentype == '(') { /* ( dcl ) */
    ...
    } else if (tokentype == NAME) /* им€ переменной */
        strcpy(name, token);
    else {
...
int getch(void) /* вз€ть (возможно возвращенный) символ */
{
